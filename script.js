...
function updateCountdown() {
  ...
  if (diff > 0) {
    ...
    elements.hours.textContent = Math.floor(
      (diff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60),
    );
    elements.minutes.textContent = Math.floor(
...
toggleBtn.addEventListener("click", async () => {
  if (isPlaying) {
    ...
    try {
      ...
      alert(
        "Klik tombol play di video galeri dulu, baru musik bisa dimainkan!",
      );
    }
  }
});
...
const correctPassword = "130625"; // GANTI PASSWORD LU DI SINI
...
submitPassword.addEventListener("click", function () {
  if (passwordInput.value === correctPassword) {
    ...
    errorMessage.textContent = "Salahh wleee, kisi kisinya di tanggal paling atas";
    passwordInput.value = "";
  }
});
